...
import net.superscary.fluxmachines.api.network.ConnectorType;
...
import static net.superscary.fluxmachines.api.network.ConnectorType.*;
import static net.superscary.fluxmachines.model.BakedModelHelper.quad;
import static net.superscary.fluxmachines.model.BakedModelHelper.v;
import static net.superscary.fluxmachines.model.CablePatterns.SpriteIdx.*;
import static net.superscary.fluxmachines.model.CablePatterns.Pattern;
import static net.superscary.fluxmachines.model.CablePatterns.QuadSetting;
...
public class CableBakedModel implements IDynamicBakedModel {
    ...
    private TextureAtlasSprite getTexture(String path) {
        return Minecraft.getInstance().getTextureAtlas(InventoryMenu.BLOCK_ATLAS).apply(FluxMachines.getResource(path));
    }
    ...
    public List<BakedQuad> getQuads(@Nullable BlockState state, @Nullable Direction side, @NotNull RandomSource rand, @NotNull ModelData extraData, @Nullable RenderType layer) {
        ...
        if (side == null && (layer == null || layer.equals(RenderType.solid()))) {
            // Called with the block state from our block. Here we get the values of the six properties and pass that to
    ...
    public TextureAtlasSprite getParticleIcon() {
        return spriteNormalCable == null
                ? Minecraft.getInstance().getTextureAtlas(InventoryMenu.BLOCK_ATLAS).apply(FluxMachines.getMinecraftResource("missingno"))
                : spriteNormalCable;
    }
